...
class FontFiles(object):
    ...
    def find(cls, family_name, is_bold, is_italic):
        ...
        *family_name* and the styles *is_bold* and *is_italic*.
        ...
        if cls._font_files is None:
            ...
            cls._font_files = cls._installed_fonts(family_name)
        try:
            fnt_file = cls._font_files[(family_name, is_bold, is_italic)]
        #cls._font_files = None
        except:
            cls._font_files = cls._installed_fonts(family_name)
            fnt_file = cls._font_files[(family_name, is_bold, is_italic)]
        return fnt_file
    ...
    @classmethod
    def _installed_fonts(cls,family_name):
        """
        ...
        for d in cls._font_directories():
            for key, path in cls._iter_font_files_in(d,family_name):
                fonts[key] = path
        return fonts
    ...
    @classmethod
    def _iter_font_files_in(cls, directory,family_name):
        """
        ...
        for root, dirs, files in os.walk(directory):
            for filename in files:
                ...
                if file_ext.lower() not in (".otf", ".ttf" ,".ttc"):
                    continue
                ...
                if extension.lower() == '.ttc':
                    try:

                        font = ttCollection.TTCollection(path)
                        for i in range(len(font)):
                            fnt = font[i]
                            f_name, name_in,is_bold,is_italic = cls._name_in_font(fnt, family_name)
                            if name_in:
                                yield ((family_name, is_bold, is_italic), path)
                    except IOError as e:
                        pass
                    except:
                        pass
                        # f_name, name_in, is_bold, is_italic = cls._name_in_font(font, family_name)
    ...
    @classmethod
    def _name_in_font(cls, font, family_name):
        name_in = False
        is_Italic = False
        is_Bold = False
        # try:
        JP_name = font['name'].getName(nameID=1, platformID=3, platEncID=1, langID=1041)
        Style_name = font['name'].getName(nameID=2, platformID=3, platEncID=1, langID=1033)
        EN_name = font['name'].getName(nameID=1, platformID=3, platEncID=1, langID=1033)

        if JP_name:
            JP_name = JP_name.toStr()
            name_in = True

        if Style_name:
            Style_name = Style_name.toStr()
            ...
            if 'Italic' in Style_name:
                is_Italic = True
        if EN_name:
            EN_name = EN_name.toStr()
        else:
            EN_name = font['name'].names[1].toStr()
            # print([JP_name,EN_name,is_Bold,is_Italic])
        # except:
        ...
        #         except:
        #             tmp = ""
        #         if family_name in tmp:
        #             name_in = True
        #         if 'Bold' in tmp:
        #             is_Bold = True
        #         if 'Italic' in tmp:
        #             is_Italic = True
        # f_name = font['name'].names[1]
        f_name = EN_name
        return f_name, name_in, is_Bold, is_Italic
